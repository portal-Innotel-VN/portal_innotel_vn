...
- **Blocking**: GitHub native issue dependencies, with a `Blocked by: #<n>` fallback.
- **Frontier query**: select the first unblocked, unassigned, open child in map order.
- **Claim**: `gh issue edit <n> --add-assignee @me`.
- **Resolve**: comment with the answer, close the child, then update the map’s Decisions-so-far.
