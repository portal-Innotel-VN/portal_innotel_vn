...
if (!empty($proposal->signature)) {
    $proposal->content .= '<br /><br /><span style="font-weight:bold;text-align: right;">';
    $proposal->content .= _l('proposal_signed_by') . ": {$proposal->acceptance_firstname} {$proposal->acceptance_lastname}<br />";
    $proposal->content .= _l('proposal_signed_date') . ': ' . _dt($proposal->acceptance_date) . '<br />';
    $proposal->content .= _l('proposal_signed_ip') . ": {$proposal->acceptance_ip}";
    $proposal->content .= '</span>';
}
